...
class MainScene extends Phaser.Scene {
  ...
  preload(): void {
    this.load.spritesheet('oliver', oliverSpritesheet, { frameWidth: 275, frameHeight: 400});
...
this.anims.create({
    key: 'forward',
    frames: this.anims.generateFrameNumbers('oliver', { start: 0, end: 3 }),
    frameRate: 6,
    repeat: -1
});
this.anims.create({
    key: 'backwards',
    frames: this.anims.generateFrameNumbers('oliver', { start: 4, end: 7 }),
    frameRate: 6,
    repeat: -1
});
this.anims.create({
    key: 'left',
    frames: this.anims.generateFrameNumbers('oliver', { start: 8, end: 11 }),
    frameRate: 6,
    repeat: -1
});
this.anims.create({
    key: 'right',
    frames: this.anims.generateFrameNumbers('oliver', { start: 12, end: 15 }),
    frameRate: 6,
    repeat: -1
});
...
players.forEach(player => {
 ...
 update(){
     for(let i = 0; i < players.length; i++){
        ...
        this.physics.collide(players[i], goals[i], () =>{ this.player1ReachedGoal(i)});
     }
    
    
    this.physics.collide(players[0], obstacles[0], () =>{ this.playerHitObstacle(0)})
    this.physics.collide(players[0], obstacles[1], () =>{ this.playerHitObstacle(0)})
    this.physics.collide(players[1], obstacles[2], () =>{ this.playerHitObstacle(1)})
    this.physics.collide(players[1], obstacles[3], () =>{ this.playerHitObstacle(1)})
    this.physics.collide(players[2], obstacles[4], () =>{ this.playerHitObstacle(2)})
    this.physics.collide(players[2], obstacles[5], () =>{ this.playerHitObstacle(2)})
    this.physics.collide(players[3], obstacles[6], () =>{ this.playerHitObstacle(3)})
    this.physics.collide(players[3], obstacles[7], () =>{ this.playerHitObstacle(3)})
} 
